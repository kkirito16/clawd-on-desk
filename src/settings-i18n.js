...
(function initSettingsI18n(root) {
  const STRINGS = {
    en: {
      ...
      feishuApprovalEventSubDesc: "Subscribe to the new-style card callback in the Feishu console, or button presses never reach Clawd. Expand for the steps.",
      feishuApprovalEventSubStep1Html: "1. Open [Feishu Open Platform](https://open.feishu.cn/app) → your app → Events & Callbacks.",
      feishuApprovalEventSubStep2: "2. Set the subscription method to long connection (finish step 3 first — Feishu only saves this while the long connection is online).",
      ...
      feishuApprovalEventSubStep4: "4. If Feishu asks for a release, publish a new app version, then send a test card below.",
      ...
      feishuApprovalTestNoResponse: "The test card was sent, but no button press came back within 60 seconds. This usually means the card.action.trigger callback is not subscribed in the Feishu Open Platform — check step 4.",
      feishuApprovalTestNotConnected: "Feishu long connection is not established. Check the App ID / App Secret and your network, then try again.",
      ...
      animIdleVisualDesc: "Which look the pet rests in while idle. Agent activity, sleep, and reactions still take over as usual.",
    ...
    zh: {
      ...
      feishuApprovalEventSubDesc: "需要在飞书后台订阅新版卡片回调，否则点按钮 Clawd 收不到。展开查看配置攻略。",
      feishuApprovalEventSubStep1Html: "1. 打开[飞书开放平台](https://open.feishu.cn/app) → 你的应用 →「事件与回调」。",
      feishuApprovalEventSubStep2: "2. 订阅方式选择「使用长连接接收回调」（需要先完成第 3 步启用——长连接在线时飞书才允许保存）。",
      feishuApprovalEventSubStep3: "3. 添加回调，勾选「卡片回传交互」（card.action.trigger）——不要选带「旧」字的 _v1，它不支持长连接。",
      feishuApprovalEventSubStep4: "4. 如果飞书提示需要发版，发布一个新的应用版本，然后回来发送下方的测试卡片。",
      ...
      feishuApprovalTestNoResponse: "测试卡片已发出，但 60 秒内没有收到按钮回调。通常是飞书后台没有订阅「卡片回传交互」（card.action.trigger）回调，请检查第 4 步的事件订阅。",
      feishuApprovalTestNotConnected: "飞书长连接尚未建立。请检查 App ID / App Secret 和网络后重试。",
      ...
      animIdleVisualDesc: "宠物空闲时展示的默认形象。任务动画、睡眠和互动反应仍会正常触发。",
    ...
    "zh-TW": {
      ...
      animIdleVisualDesc: "寵物閒置時顯示的預設外觀。任務動畫、睡眠與互動反應仍會照常觸發。",
    ...
    ko: {
      ...
      animIdleVisualDesc: "펫이 대기 중일 때 보여줄 기본 모습입니다. 작업 애니메이션, 수면, 반응은 평소처럼 우선 적용됩니다.",
    ...
    ja: {
      ...
      animIdleVisualDesc: "待機中に表示される見た目です。タスクアニメーション・スリープ・リアクションは通常どおり優先されます。",
